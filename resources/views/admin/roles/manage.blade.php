                            ...
                            <tr>
                                    ...
                                    <div class="role-list">
                                        @forelse($user->roles as $role)
                                            <div class="role-item">
                                                {{ $role->nama_role }}</div>
                                        @empty
                                            <span style="color: var(--text-muted); font-style: italic;">— Belum ada role
                                                —</span>
                                        @endforelse
                                    </div>
                                </td>
                                <td class="actions">
                                    @foreach ($user->roles as $role)
                                        <form
                                            action="{{ route('admin.roles.assign.remove', ['userId' => $user->id, 'roleId' => $role->idrole]) }}"
                                            method="POST"
                                            onsubmit="return confirm('Hapus role \'{{ $role->nama_role }}\' dari \'{{ $user->nama }}\'?')">
                                            @csrf
                                            @method('DELETE')
